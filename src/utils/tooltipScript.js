if (typeof $WH == "undefined") {
  ...
  /* custom */
  let g_staticUrl = 'https://tbcdb.com/tooltips';
}

if (typeof $WowheadPower == "undefined") {
  let $WowheadPower = new function () {
    let isRemote = $WH.wowheadRemote;
    let
      opt  = { applyto: 3 },
    ...
    function init() {
      if (isRemote) {
        ...
        script.src = g_staticUrl + "/basic.js?123";
    ...
    function initCSS() {
      ...
      for (let k in aowow_tooltips.hide) {
    ...
    this.init = function () {
      if (isRemote) {
        $WH.ae(head, $WH.ce("link", {
          ...
          href: g_staticUrl + "/basic.css?123",
          rel:  "stylesheet"
        }));
      }
      attachEvent();
      ...
      if (url) {
        ...
        if ($WH.in_array(['fr', 'de', 'cn', 'es', 'ru', 'en'], domain) == -1) {
          for (i in document.scripts) {
            ...
            let dmn = document.scripts[i].src.match(/widgets\/power.js\?(lang|locale)=(en|fr|de|cn|es|ru)/i);
            if (dmn) {
              domain = dmn[2];
        ...
        let data = LOOKUPS[type][0][getFullId(typeId, params)];
        ...
        let timeout = function (t) {
          ...
          if (aowow_tooltips.iconizelinks && (type == TYPE_ITEM || type == TYPE_ACHIEVEMENT || type == TYPE_SPELL) && data.icon) {
            ...
            t.style.background = "url(https://tbcdb.com/images/icons/medium/" + data.icon.toLocaleLowerCase() + ".jpg) left center no-repeat";
            t.style.backgroundSize = "15px 15px"
          }
    ...
    function display(type, id, locale, params) {
      ...
      let fullId    = getFullId(id, params);
      currentType   = type;
      currentId     = fullId;
    ...
    function request(type, id, locale, stealth, params) {
      ...
      // let url = "http://" + $WH.g_getDomainFromLocale(locale) + ".wowhead.com"
      // let url = (document.location.protocol != "https:" ? "http:": document.location.protocol) + "//" + localeDomain + ".wowhead.com";
      let
        localeDomain = $WH.g_getDomainFromLocale(locale);

      $WH.g_ajaxIshRequest("https://tbcdb.com/ajax.php?" + LOOKUPS[type][1] + "=" + id + "&power" + p);
      //$WH.g_ajaxIshRequest("https://tbcdb.com/ajax.php?" + LOOKUPS[type][1] + "=" + id + "&domain=" + localeDomain + "&power" + p);
      if (SCALES[type] && SCALES[type][locale] == SCALES_NONE) {
        $WH.g_ajaxIshRequest('https://wotlkdb.com/' + SCALES[type].url);
      ...
      if (!html) {
        ...
        if (currentParams != null) {
          if (currentParams.pcs && currentParams.pcs.length) {
            let n = 0;
            for (let i = 0, len = currentParams.pcs.length; i < len; ++i) {
              let m;
          ...
          if ((currentType == TYPE_ACHIEVEMENT) && currentParams.cri) {
            for (let i = 0; i < currentParams.cri.length; i++) {
